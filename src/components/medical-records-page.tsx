    ...
    <form ref={formRef} onSubmit={submit} noValidate className="relative space-y-5" aria-busy={saving || !draftReady}>
      ...
      <SectionCard title="基本情報" description="必須項目には「必須」と表示しています。"><div className="space-y-4">
        ...
        {form.hasNextVisit ? <div className="space-y-4 rounded-2xl bg-teal-50/60 p-3">
          <Field label="予約する期限" required error={errors.reservationDeadline}><div className="input flex items-center gap-2"><input aria-invalid={Boolean(errors.reservationDeadline)} type="date" value={form.reservationDeadline ?? ""} onChange={(e) => update({ reservationDeadline: e.target.value || null })} className="min-w-0 flex-1 bg-transparent outline-none" /><span className="shrink-0 text-sm font-bold text-teal-800">{formatWeekday(form.reservationDeadline ?? "")}</span></div></Field>
          ...
          {form.reservationStatus === "booked" ? <div className="space-y-4 rounded-xl border border-teal-100 bg-white p-3">
            <Field label="予約日時" required error={errors.appointmentDateTime}><div className="input flex items-center gap-2"><input aria-invalid={Boolean(errors.appointmentDateTime)} type="datetime-local" value={form.appointmentDateTime ?? ""} onChange={(e) => update({ appointmentDateTime: e.target.value || null, appointmentDateJst: e.target.value ? e.target.value.slice(0, 10) : null })} className="min-w-0 flex-1 bg-transparent outline-none" /><span className="shrink-0 text-sm font-bold text-teal-800">{formatWeekday(form.appointmentDateTime?.slice(0, 10) ?? "")}</span></div></Field>
